        ...
        <div className="relative mb-8 md:mb-16">
            <ContentWrapper>
                ...
                {!loading ? (
                    ...
                    {/* {console.log(data)} */}
                        {data?.length===0 ? <div className="text-white text-sm md:text-base italic opacity-75">No Screenshots</div> : data?.map((image,idx) => {
                            {/* console.log(video) */}
                            let imgUrl = (image?.file_path) ? url.poster+image?.file_path : ScreenshotFallback;
                            return (
                                <div key={idx} className="w-[200px] aspect-video flex-shrink-0 md:w-[calc(25%-15px)]" >
